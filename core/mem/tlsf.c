 ...
 * @date    08.02.2014.
 *
 * @author Matthew Conte <matt@baisoku.org>
 * @copyright Public Domain, no restrictions.
 * @source http://tlsf.baisoku.org
 */
...
tlsf_static_assert(sizeof(int) * __CHAR_BIT__ == 32);
tlsf_static_assert(sizeof(size_t) * __CHAR_BIT__ >= 32);
tlsf_static_assert(sizeof(size_t) * __CHAR_BIT__ <= 64);

/* SL_INDEX_COUNT must be <= number of bits in sl_bitmap's storage type. */
tlsf_static_assert(sizeof(unsigned int) * __CHAR_BIT__ >= SL_INDEX_COUNT);
...
static void remove_free_block(control_t * control, block_header_t * block,
                              int fl, int sl) {
...
static void insert_free_block(control_t * control, block_header_t * block,
                              int fl, int sl) {
...
void tlsf_walk_pool(pool_t pool, tlsf_walker walker, void *user) {
    ...
    while(block && !block_is_last(block)) {
        pool_walker(block_to_ptr(block),
                    block_size(block), !block_is_free(block), user);
...
#    endif
    ...
    if(rv) {
        kprintf("tlsf_create: %x ffs/fls tests failed!\n", rv);
    }
    return rv;
}
#endif
...
void *tlsf_malign(tlsf_t tlsf, size_t size, size_t align) {
    ...
    if(block) {
        ...
        size_t gap = tlsf_cast(size_t,
                               tlsf_cast(tlsfptr_t,
                                         aligned) - tlsf_cast(tlsfptr_t, ptr));
        ...
        if(gap && gap < gap_minimum) {
            ...
            gap = tlsf_cast(size_t,
                            tlsf_cast(tlsfptr_t, aligned) - tlsf_cast(tlsfptr_t,
                                                                      ptr));
        }
